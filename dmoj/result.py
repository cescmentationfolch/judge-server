...
class Result:
    ...
    def update_feedback(self, error, process, binary, feedback=None):
        self.feedback = (feedback or (process.feedback if hasattr(process, 'feedback') else
                         getattr(binary, 'get_feedback', lambda x, y, z: '')(error, self, process)))
        if not self.feedback and self.get_main_code() == Result.RTE:
            if not process.was_initialized:
                ...
                self.feedback = 'failed initializing'
            else:
                self.feedback = strsignal(process.signal).lower()
